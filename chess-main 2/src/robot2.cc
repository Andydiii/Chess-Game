        ...
        for (int j = 0; j < 8; j++) {
            ...
            for (int desti = 0; desti < 8; desti++) {
                for (int destj = 0; destj < 8; destj++) {
                    ...
                    bool canCheck, canCapture, fake;

                    if (p != nullptr && p->getType() == "pawn" && (desti == 0 || desti == 7)) {
                        // pawn promotion
                        if (gameBoard->validmove(team, pos, dest, canCheck, canCapture, fake, fake, "queen")) {
                        ...
                        if (gameBoard->validmove(team, pos, dest, canCheck, canCapture, fake, fake, "knight")) {
                    ...
                    } else if (gameBoard->validmove(team, pos, dest, canCheck, canCapture, fake, fake, "DNE")) {
                        // not pawn promotion
